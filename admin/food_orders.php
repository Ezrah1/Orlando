<?php
$page_title = 'Food Orders Management';
...
<div class="page-header">
    <h1 class="page-title">Food Orders Management</h1>
</div>
    ...
    if(isset($_POST['action'])) {
        switch($_POST['action']) {
            case 'create_order':
                ...
                $order_number = 'FO-' . date('Ymd') . '-' . str_pad(rand(1, 999), 3, '0', STR_PAD_LEFT);
                
                $sql = "INSERT INTO food_orders (order_number, table_number, room_number, guest_name, guest_phone, order_type, special_instructions, ordered_by) 
                        VALUES ('$order_number', '$table_number', '$room_number', '$guest_name', '$guest_phone', '$order_type', '$special_instructions', " . $_SESSION['user_id'] . ")";
                ...
                if(mysqli_query($con, $sql)) {
                    ...
                    if(isset($_POST['menu_items']) && is_array($_POST['menu_items'])) {
                        foreach($_POST['menu_items'] as $item) {
                            ...
                            // Get cost price for this menu item
                            $cost_query = "SELECT cost_price FROM menu_items WHERE id = $menu_item_id";
                            $cost_result = mysqli_query($cost_query, "");
                            $cost_row = mysqli_fetch_assoc($cost_result);
                            $cost_price = $cost_row['cost_price'] ?? 0;
                            $total_cost = $quantity * $cost_price;
                            
                            mysqli_query($con, "INSERT INTO order_items (order_id, menu_item_id, quantity, unit_price, total_price, cost_price, total_cost, special_instructions) 
                                               VALUES ($order_id, $menu_item_id, $quantity, $unit_price, $total_price, $cost_price, $total_cost, '$special_instructions')");
                        }
                    }
                    
                    // Update order totals
                    $total_query = "SELECT SUM(total_price) as total FROM order_items WHERE order_id = $order_id";
                    $total_result = mysqli_query($total_query, "");
                    $total_row = mysqli_fetch_assoc($total_result);
                    $total_amount = $total_row['total'] ?? 0;
                    
                    mysqli_query($con, "UPDATE food_orders SET total_amount = $total_amount, final_amount = $total_amount WHERE id = $order_id");
            ...
            case 'update_item_status':
                ...
                $update_fields = "status = '$status'";
                if($status == 'ready') {
                    $update_fields .= ", prepared_at = NOW(), prepared_by = " . $_SESSION['user_id'];
                }
                
                $sql = "UPDATE order_items SET $update_fields WHERE id = $item_id";
                if(mysqli_query($con, $sql)) {
...
if($status_filter) {
    $where_clause .= " AND fo.status = '$status_filter'";
}
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Create New Food Order</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div id="orderItems">
                            <div class="order-item-row">
                                <div class="row">
                                    <div class="col-md-4">
                                        <select name="menu_items[0][menu_item_id]" class="form-control menu-item-select" required>
                                            ...
                                            ?>
                                                <option value="<?php echo $item['id']; ?>" data-price="<?php echo $item['price']; ?>">
                                                    <?php echo $item['name']; ?> - KES <?php echo number_format($item['price']); ?>
                                                </option>
                                            <?php endwhile; ?>
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                                        <input type="number" name="menu_items[0][quantity]" class="form-control quantity-input" value="1" min="1" required>
                                    </div>
                                    <div class="col-md-2">
                                        <input type="number" name="menu_items[0][unit_price]" class="form-control price-input" step="0.01" readonly>
                                    </div>
                                    <div class="col-md-3">
                                        <input type="text" name="menu_items[0][special_instructions]" class="form-control" placeholder="Special instructions...">
                                    </div>
                                    <div class="col-md-1">
